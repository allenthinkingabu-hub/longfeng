...
**compliance check**：
```bash
bash .Codex/skills/fe-builder/scripts/check-compliance.sh <CSS_FILE> <TSX_FILE>
```

**pixel diff**（V 类修复后验证）：
```bash
node .Codex/skills/fe-accept-mock/scripts/pixel-diff.js \
  "<MOCKUP_HTML_PATH>" \
...
> 完整规则详见 `.Codex/skills/fe-builder/references/compliance-rules.md`
...
d. 全页回归：
   bash .Codex/skills/fe-builder/scripts/check-compliance.sh <CSS> <TSX>
