...
test.describe('SC-01 · 拍题入库 @sc-01', () => {
  ...
  test('异常 · presign 失败 → 显示错误 banner，不进入 P03', async ({ studentPage }) => {
    // B 轨：拦截 presign 返回 500
    await studentPage.route('**/api/v1/files/presign', (route) => route.fulfill({ status: 500, body: '{"error":"presign_failed"}' }));
