 ...
 *   <li>{@link Resource} → {@link TempFileSpooler#spool} 立即落盘 → {@link FileSystemResource} 喂 ChatClient
 ...
 *
 * <pre>
 *   spool(image) → NSFW 过滤 → face mask → ChatClient.prompt()
 *      .user(prompt + media(MimeType.IMAGE_JPEG, fileResource))
 *      .advisors(PromptInjectionGuardAdvisor)
 *      .call()
 *      .entity(AnalysisResult.class)   ← Spring AI 结构化输出
...
public class QuestionAnalyzerImpl implements QuestionAnalyzer {
  ...
  /**
   * 核心 LLM 调用 · 同步阻塞 · 调用方包 Mono.fromCallable 切线程。
   *
   * @param taskId      任务 ID
    ...
    try {
      ...
      return fallbackOrchestrator.tryWithFallback(
          ...
          provider -> {
            ...
            FileSystemResource fileResource = new FileSystemResource(finalTmp);
            MimeType mime = MimeTypeUtils.parseMimeType("image/jpeg");
            return client
                .prompt()
                .user(
                    u ->
                        u.text(promptTemplate)
                            .params(
                                Map.of(
                                    ...
                                    "ocrHint",
                                    ""))
                            .media(mime, fileResource))
                .advisors(injectionGuard)
                .call()
